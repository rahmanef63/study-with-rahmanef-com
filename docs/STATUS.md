...
| 31 | KONTEN: kursus "AI untuk Produktivitas Kerja" (`ai-produktivitas-kerja`, `convex/seedAiKerja.ts`) | v1.5 | #11 | in-progress | beta | — | — | pre-claimed; wave KONTEN — pola seedWebDev, idempoten, 5 modul/12–14 lesson/4 kuis |
| 32 | KONTEN: kursus "Analisis Data dengan AI" (`analisis-data-dengan-ai`, `convex/seedAnalisisData.ts`) | v1.5 | #11 | in-progress | gamma | — | — | pre-claimed; wave KONTEN |
| 33 | KONTEN: kursus "Orkestrasi Multi-Agent" (`orkestrasi-multi-agent`, `convex/seedMultiAgent.ts`) | v1.5 | #11 | in-progress | delta | — | — | pre-claimed; wave KONTEN; studi kasus = platform ini sendiri |
| 34 | ops: deploy modul seed + run 3 seed di prod + smoke kursus baru | v1.5 | #31–#33 | open | vps | — | — | idempoten; angka saja; JANGAN sentuh data user |
| 35 | v1.6 (BACKLOG, keputusan owner 2026-07-16): asisten belajar AI in-OS — cakupan tutor materi (konteks lesson) + chat umum; AKSES: tanpa kuota di awal (alpha WAJIB angkat ulang guard biaya saat build); butuh env ANTHROPIC_API_KEY di prod (owner set sendiri via `npx convex env set` — nilai tak pernah lewat repo/chat) | v1.6 | #34 | open | alpha | — | — | OS-7 unblocked by owner; desain saat wave dibuka |
