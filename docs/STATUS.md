...
- **[vps → zeta/alpha] e2e spec 8 console-error whitelist** — `collectErrors` di `e2e/smoke.anon.spec.ts` harus mem-filter log expected `ConvexError … NOT_FOUND` (Convex client SELALU `console.error` server error) + `[profiles:PublicProfileBoundary] NOT_FOUND` di flow sertifikat-bogus; spec-nya gagal terhadap backend nyata padahal kontrak UX lolos (copy not-found tampil, no crash). Lihat docs/reports/vps-2026-07-16.md.
- **[vps → alpha/owner] handle profil `rahman` tidak ada di prod** — smoke/docs pakai `/profil/rahman` tapi handle real admin = `abdurrahman-fakhrul` (route 200 fail-soft). Seed handle `rahman` ATAU update docs/e2e ke handle real biar smoke profil benar-benar menguji profil hidup.
