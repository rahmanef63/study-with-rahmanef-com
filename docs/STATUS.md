...
| 15 | UI/UX: PRD + design exploration (agent ui) + wave polish UI-A/B/C | v1.2 | #14 | in-progress | ui | — | — | Arc besar SHIPPED (UI-A/B/C + Editorial Warmth + OS pivot — mayoritas oleh owner+alpha). SISA = closing pass wave v1.4: polish permukaan v1.3 (cari/notifikasi/sertifikat/bell/gate) + UI-UX-PRD v3.2 + tutup arc — prompt di AGENT-PROMPTS.md |
...
| 28 | notifikasi pengumuman — fan-out bounded ke member (kind `announcement`) | v1.4 | #10, #21 | in-progress | beta | — | — | pre-claimed; schema union by alpha; internal `createMany` (cap ≤200) + producer di announcements.create; no self-notify |
| 29 | pencarian meluas ke papan sumber (kind `resource`, approved-only) | v1.4 | #23 | in-progress | gamma | — | — | pre-claimed; TANPA search index baru (by_tenant_status + filter memori, bounded); klik = url eksternal |
| 30 | ops: deploy v1.4 + smoke (finale) | v1.4 | #15, #28, #29 | open | vps | — | — | Cloud deploy (schema union additive); ingatkan #12 |
