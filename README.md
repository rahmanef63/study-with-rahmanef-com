...
A platform for learning applied AI, shaped as a **desktop OS in the browser** — free, multi-tenant, self-hosted. Discord-first, YouTube-embed. Charity project by Rahman.

> Every path opens a windowed desktop (macOS · Windows · iOS · Android · Dashboard) that wraps the learning slices as apps. The Convex backend is unchanged — only the frontend chrome became an OS.
