...
- **Branch flow** (amends rr solo-dev direct-to-main for parallel work): work on `slice/<slug>` or `app/<area>`. Conventional commits; end AI commits with `Co-Authored-By: Claude <noreply@anthropic.com>`. The integrator merges to main. Never push to main unless you are the integrator session.
