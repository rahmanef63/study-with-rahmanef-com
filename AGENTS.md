...
Next.js ^16 · React ^19 · Tailwind v4 (`@tailwindcss/postcss`) · Convex Cloud (`rare-toucan-552`; self-hosted retired 2026-07-10) · `@convex-dev/auth` (NO Clerk) · TypeScript strict · `proxy.ts` (never `middleware.ts`). Do not add or upgrade any dependency without (a) checking the rr catalog first and (b) logging a proposal in `docs/STATUS.md`.
